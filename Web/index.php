...
if(isset($currentTournaments) && (count($currentTournaments) > 0)){
	...
	for($i = 0; $i < count($currentTournaments); ++$i){
		...
		if($currentTournaments[$i]->AnnouncementOnly){
			...
			echo '<td style="border:none; width:30%">' . GetUnbreakableDash($currentTournaments[$i]->Name) . '</td>'. PHP_EOL;
			...
			if($now < $startSignUp){
				...
				echo '<td style="border:none"><a href="' . $script_folder_href . 'signup.php?tournament=' . $currentTournaments [$i]-> TournamentKey . '">Sign up</a> (<span style="font-size:small;">ends ' . $endSignUpFriendlyDate . '</span>)</td>'. PHP_EOL;
